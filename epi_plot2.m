function epi_plot(Data, channel_type, f_low, f_high, cortex, cluster, spike_ind, ...
    picked_components, picked_comp_top, spike_sources, bf_ts, MRI, G3, channels, corr_thresh, hemi)
%     UseInterpolation,  DataFile, spike_source, datatime, 
%      G2, Ff, Fs, namechan)
...
% Spike timeseries on sensors
clear spike_trials
for i = 1:length(cluster)
    for j = 1:size(cluster{1,i}, 2) % number of spikes in the cluster
        indtime = cluster{1,i}(2,j); % spike timesamples
        spike_trials{i}(:,:,j) = Ff(:,(indtime-40):(indtime+120)); % spikes from recordings
    end
    
    spike_av{i} = mean(spike_trials{i}, 3);
    [val ind] = sort(abs(spike_av{i}(:,41)), 'descend'); % sort by peak amplitude
    channels_maxamp{i} = ind(1:20);
    maxamp_spike_av{i} = spike_av{i}(ind(1:10,:),:); % ten channels with the highest amplitude
    for j = 1:10
        if maxamp_spike_av{i}(j,41) > 0
            maxamp_spike_av{i}(j,:) = -maxamp_spike_av{i}(j,:);
        end
    end
    clust_center(i,:) = mean(R(cluster{i}(1,:),:));
end

% MAIN FIGURE
% Create the main figure with detected clusters
c = [lines(7); 0.15, 0.15, 0.15; ...
    0, 0.5, 0.5; 0.3, 0.18, 0.4; ...
    0.8, 0.08, 0.5; 0.9, 0.5, 0.5;  0.1, 0.3, 0.2; lines(7); 0.15, 0.15, 0.15; ...
    0, 0.5, 0.5; 0.3, 0.18, 0.4; ...
...
% end
% if nargin < 15
%    bmf_ts = zeros(size(cluster,2), size(Ff,2));
%    setappdata(h, 'beamformer', 0);
% else setappdata(h, 'beamformer', 1);
% end
...
%     mask_lr, 0.05)
f = scatter3(R(1,1), R(1,2), R(1,3), 'MarkerEdgeAlpha', 0.2)
axis equal
grid off
axis off
hold on
...
function cluststatistics(source, event)
    ...
    [val ind] = min(vecnorm((repmat(loc, length(cluster), 1)-clust_center)'));
    if val < 1
%     clust_num = find(sum(source.MarkerFaceColor == c, 2)==3);
        clust_num = ind;
    end
    ...
    title('Disctribution of subcorrs')
    set(gca,'fontsize', 14)
    ...
    cla(h3)
    h3 = subplot(3,4,3);
    plot(-40:120, maxamp_spike_av{clust_num}', 'Color', c(clust_num,:), 'LineWidth', 2)
    hold on
    xlim([-40 120])
    title('Average spike on 10 top amplitude sensors')
    set(gca,'fontsize', 14)
    ...
    h4 = subplot(3,4,4);
    spike_av = zeros(1, 79);
    for i = 1:length(cluster{1, clust_num})
        int = 20:100;
        [val, ind] = max(spike_sources{clust_num}(i,int));
        spike_moved = spike_sources{clust_num}(i,(int(ind)-19):(int(ind)+59));
        plot(-19:59, spike_moved, 'Color', c(clust_num,:))
        hold on
        spike_av = spike_av + spike_moved;
    end
    plot(-19:59, spike_av/length(cluster{1, clust_num}), 'k', 'LineWidth', 2)
    title('Events timeseries (sources)')
    xlim([-20 60])
